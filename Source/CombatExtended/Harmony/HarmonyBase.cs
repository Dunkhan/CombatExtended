...
using CombatExtended.CombatExtended.Things;
...
namespace CombatExtended.Harmony
{
    public static class HarmonyBase
    {
        ...
        private static void MultiplayerCompatibility()
        {
            ...
            MultiplayerSynchroniser.IsActive = true;
            MethodInfo original = AccessTools.Method("Multiplayer.Client.SyncHandlers:Init");
            instance.Patch(original, new HarmonyMethod(SymbolExtensions.GetMethodInfo(() => PatchMultiPlayerSyncMethod())));

        }

        private static void PatchMultiPlayerSyncMethod()
        {
            AccessTools.Method("Multiplayer.Client.SyncMethod:Register").Invoke(null, new object[] { typeof(MultiplayerSynchroniser), nameof(MultiplayerSynchroniser.SyncTryStartReload), null });
        }
